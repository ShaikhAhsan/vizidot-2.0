  ...
  try {
    ...
    if (isGCSAvailable()) {
      try {
        ...
        console.error('Firebase Storage upload failed, falling back to local storage:', firebaseError.message);
        console.error('Firebase Storage error stack:', firebaseError.stack);
        // If production requires GCS-only, do not fall back
        if (process.env.UPLOAD_REQUIRE_GCS === 'true') {
          await require('fs').promises.unlink(req.file.path).catch(() => {});
          return res.status(503).json({
            success: false,
            error: 'Google Cloud Storage is required but upload failed. Try again or check server logs.',
      ...
      if (process.env.UPLOAD_REQUIRE_GCS === 'true') {
        return res.status(503).json({
          success: false,
          error: 'Google Cloud Storage is not configured. Set FIREBASE_SERVICE_ACCOUNT_JSON and ensure Firebase Storage is initialized.'
        });
      }
    }
