...
// Database configuration - Coolify container may resolve hostnames to 127.0.1.1; use IP to bypass
// Priority: VIZIDOT_DB_HOST_IP (raw IP, no DNS) > VIZIDOT_DB_HOST > DB_FORCE_HOST > DB_HOST
let dbHost = process.env.DB_HOST || 'c1109547.sgvps.net';

if (process.env.VIZIDOT_DB_HOST_IP && !/^127\.|^localhost$/i.test(process.env.VIZIDOT_DB_HOST_IP)) {
  dbHost = process.env.VIZIDOT_DB_HOST_IP;
  console.log('🔒 Using VIZIDOT_DB_HOST_IP (bypass DNS):', dbHost);
} else if (process.env.VIZIDOT_DB_HOST) {
  dbHost = process.env.VIZIDOT_DB_HOST;
  console.log('🔒 Using VIZIDOT_DB_HOST:', dbHost);
} else if (process.env.DB_FORCE_HOST) {
  dbHost = process.env.DB_FORCE_HOST;
  console.log('🔒 Using DB_FORCE_HOST:', dbHost);
} else if (process.env.DB_HOST_IP && !/^127\.|^localhost$/i.test(process.env.DB_HOST_IP)) {
  dbHost = process.env.DB_HOST_IP;
  console.log('🌐 Using DB_HOST_IP:', dbHost);
}

// Safety: never use localhost when external host is set
if (/^127\.|^localhost$/i.test(dbHost)) {
  const external = process.env.VIZIDOT_DB_HOST_IP || process.env.VIZIDOT_DB_HOST || process.env.DB_FORCE_HOST || process.env.DB_EXTERNAL_HOST;
  if (external) {
    dbHost = external;
    console.log('⚠️ Overrode 127.0.1.1 with:', dbHost);
  }
}
